...
export default function OrdersPage() {
  ...
  const loadData = React.useCallback(
    async (p?: number) => {
      setLoading(true);
      try {
        const res = await orderService.getPaged(
          p ?? page,
          pageSize,
          fromDate || undefined,
          toDate || undefined,
  ...
  React.useEffect(() => {
    ...
    return () => {
      signalRService.off("ReceiveOrderUpdate", handler);
    };
  }, [loadData]);
  ...
  const handleSetUnpaid = async (orderId: string) => {
    setStatusUpdating(orderId);
    try {
      await orderService.setUnpaid(orderId);
  ...
  const viewDetail = async (order: Order) => {
    try {
      const [detail, receiptData] = await Promise.all([
        orderService.getById(order.id),
        orderService.getReceipt(order.id),
      ]);
  ...
  const printReceipt = () => {
    if (!receipt) return;
    const popup = window.open("", "receipt", "width=420,height=720");
    if (!popup) {
    ...
    const lines = [
      receipt.branchName || "Restaurant",
      receipt.branchAddress || "",
      `Receipt ${receipt.orderCode}`,
      `Paid: ${receipt.paidAt ? formatDateTime(receipt.paidAt) : "Not paid"}`,
      "",
      ...receipt.items.map(
        (item) =>
  ...
  const menuItemNames =
    selectedOrder?.items?.map((item) => {
      const name =
        item.menuItemName ??
        item.comboName ??
        `Item #${item.menuItemId ?? item.comboId}`;
      const addons = item.addons
        ?.map((a) => `${a.addonName ?? "Topping"} x${a.quantity}`)
        .join(", ");
      return {
        name,
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {selectedOrder?.sourceName &&
                `Source: ${selectedOrder.sourceName}`}
            </DialogDescription>
          </DialogHeader>
          {selectedOrder && (
            <div className="space-y-6">
              ...
              <div className="space-y-1 text-right">
                <div className="text-sm text-muted-foreground">
                  Subtotal: {formatCurrency(selectedOrder.subtotalAmount)}
                </div>
                ...
                <div className="text-sm text-muted-foreground">
                  VAT: {formatCurrency(selectedOrder.vatAmount ?? 0)}
                </div>
              ...
              {receipt && (
                <div className="rounded-lg border p-3 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    <h4 className="font-medium">Receipt references</h4>
                  ...
                  <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1">
                    ...
                    <dd className="break-all font-mono text-xs">
                      {receipt.paymentIntentId || "Not available"}
                    </dd>
                    <dt className="text-muted-foreground">Provider</dt>
                    <dd>{receipt.provider || "N/A"}</dd>
                    <dt className="text-muted-foreground">
                      Provider reference
                    </dt>
                    <dd className="break-all font-mono text-xs">
                      {receipt.providerReference || "Not available"}
                    </dd>
                  </dl>
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {(STATUS_FLOW[selectedOrder.status] ?? []).map(
                    (nextStatus) => (
                      <Button
                        key={nextStatus}
                        size="sm"
                  ...
                  {selectedOrder.status === "paid" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleSetUnpaid(selectedOrder.id)}
                      disabled={statusUpdating === selectedOrder.id}
                    >
                      Mark as Unpaid
                    </Button>
