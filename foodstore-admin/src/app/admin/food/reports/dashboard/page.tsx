...
export default function FinancialReportsPage() {
  ...
  React.useEffect(() => {
    organizationService
      ...
      .then((items) => {
        ...
        setBranchId(
          items[0]?.branches.find((branch) => branch.isActive)?.id || "",
        );
      })
  ...
  async function load() {
    ...
    try {
      setReport(
        await financialReportService.get(
          organizationId,
          fromDate,
          toDate,
          branchId || undefined,
  ...
  const columns: ColumnDef<BranchFinancialSummary>[] = [
    ...
    { accessorKey: "orders", header: "Orders" },
    {
      id: "conversion",
      header: "Conversion",
      cell: ({ row }) => `${row.original.paymentConversionRate}%`,
    },
    {
      id: "gross",
      header: "Gross sales",
    ...
    },
    {
      id: "wallet",
      header: "Wallet",
      cell: ({ row }) => formatCurrency(row.original.walletPosition),
    },
  ];
  const totalCards = report
    ? [
        ...
        ["Wallet position", report.totals.walletPosition],
        ["Pending settlement", report.totals.pendingSettlement],
      ]
    : [];
  return (
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="grid gap-2">
            <Label htmlFor="from">From (Kenyan date)</Label>
            <input
              id="from"
              type="date"
              value={fromDate}
          ...
          <div className="grid gap-2">
            <Label htmlFor="to">To (Kenyan date)</Label>
            <input
              id="to"
              type="date"
              value={toDate}
          ...
          <Button
            ...
            disabled={!report}
            onClick={() =>
              void financialReportService
                .export(organizationId, fromDate, toDate, branchId || undefined)
          ...
          <Button
            ...
            onClick={() =>
              void financialReportService
                .exportPdf(organizationId, fromDate, toDate, branchId || undefined)
                .catch((error) => toast.error((error as Error).message))
            }
          >
        ...
        {report && (
          <>
              ...
              {totalCards.map(([label, amount]) => (
                <Card key={String(label)}>
                  ...
                  <CardContent className="text-2xl font-bold">
                    {formatCurrency(amount as number)} {report.currency}
                  </CardContent>
                </Card>
              ))}
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              <Card>
                <CardHeader>
                  <CardTitle>Payment conversion</CardTitle>
                </CardHeader>
                <CardContent className="text-2xl font-bold">
                  {report.totals.paymentConversionRate}%
                </CardContent>
              </Card>
              <Card>
                <CardHeader>
                ...
                <CardContent>
                  {report.totals.settlementCount} batches
                </CardContent>
              </Card>
              <Card>
                <CardHeader>
                ...
                <CardContent className="text-sm">
                  {report.exceptions.failedPayments} failed ·{" "}
                  {report.exceptions.refundsAwaitingAction} refunds pending ·{" "}
                  {report.exceptions.settlementFailures} settlement failures ·{" "}
                  {report.exceptions.reconciliationDiscrepancies} discrepancies
                </CardContent>
              </Card>
            </div>
            <Card>
