...
export default function SettlementsDashboardPage() {
  ...
  const generate = async () => {
    ...
    try {
      await settlementService.generate(orgId, new Date().toISOString().slice(0, 10), org.currencyCode)
  ...
  const advance = async () => {
    ...
    try {
      await settlementService.transition(selected.id, target, payoutRef || undefined)
  ...
  const columns: ColumnDef<SettlementBatch>[] = [
    ...
    { id: "commission", header: "Commission", cell: ({ row }) => formatCurrency(row.original.commissions) },
    { id: "net", header: "Net payout", cell: ({ row }) => formatCurrency(row.original.netAmount) },
    {
      id: "status",
      header: "Status",
    ...
    },
    {
      id: "actions",
      header: "",
      cell: ({ row }) => <Button variant="ghost" size="sm" onClick={() => detail(row.original)}>Statement</Button>,
    },
  ]
  ...
  return (
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
        ...
        <DataTable columns={columns} data={batches} loading={loading} searchKey="periodDate" searchPlaceholder="Search settlement periods..." />
      </div>
      ...
      <CrudSheet
        ...
        onSubmit={advance}
        submitLabel={selected && nextState[selected.status] ? `Mark ${nextState[selected.status].replace("_", " ")}` : "No action"}
      >
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <Badge variant="secondary">{selected?.status ?? "draft"}</Badge>
            {selected && (
              <Button type="button" variant="outline" size="sm" onClick={() => window.open(settlementService.exportUrl(selected.id), "_blank", "noopener,noreferrer")}>
          ...
          {nextState[selected?.status ?? ""] === "paid" && (
            <div className="grid gap-2">
              ...
              <Input id="payoutReference" value={payoutRef} onChange={e => setPayoutRef(e.target.value)} placeholder="Bank transfer / payout reference" />
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            {selected?.lines?.map(line => (
              <div className="rounded border p-2 text-xs" key={line.id}>
                ...
                {line.paymentIntentId} - gross {formatCurrency(line.grossAmount)}, net {formatCurrency(line.netAmount)}
              </div>
            ))}
          </div>
        </div>
