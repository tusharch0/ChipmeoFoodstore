...
public sealed class SettlementsController(ISettlementService service, IHubContext<AppHub> hub, ITenantAccessService tenantAccess) : ControllerBase
{
    ...
    public async Task<IActionResult> Export(Guid id, CancellationToken ct)
    {
        ...
        csv.AppendLine();
        csv.AppendLine("Payment Intent ID,Provider,Provider Reference,Gross Amount,Commission Amount,Provider Fee Amount,Net Amount");
        foreach (var line in batch.Lines.OrderBy(l => l.CreatedAt))
            csv.AppendLine($"{line.PaymentIntentId},{Escape(line.Provider)},{Escape(line.ProviderReference)},{line.GrossAmount},{line.CommissionAmount},{line.ProviderFeeAmount},{line.NetAmount}");
